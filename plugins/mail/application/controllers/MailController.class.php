...
class MailController extends ApplicationController {
	...
	function list_all() {
		...
		$attributes = array(
			...
			"search" => array_var($_GET,'text_filter'),
		);
		...
		// Si hay búsqueda, asegurar join con mail_datas
    	if (!empty($attributes['search']) && empty($join_params)) {
        $join_params = array(
            'table' => TABLE_PREFIX.'mail_datas',
            'jt_field' => 'id',
            'e_field' => 'object_id',
...
private function getEmails(
    ...
    $only_count_result = null,
    $extra_cond = ""
) {

    if (!array_var($_REQUEST, 'dont_override_account_filter')) {
        Hook::fire('override_email_account_filter', [
            'context' => $context,
            'start' => $start,
            'limit' => $limit,
            'order_by' => $order_by,
    ...
    Hook::fire('override_email_list_filters', [
        'context' => $context,
        'start' => $start,
        'limit' => $limit,
        'order_by' => $order_by,
    ...
    // Retorna null si no se deben mostrar emails
    if (!isset($attributes["viewType"]) || !in_array($attributes["viewType"], ["all","emails"])) {
        return null;
    }
    ...
    if (!empty($attributes['search'])) {
        ...
        $searchValue = preg_replace('/[@\[\]\(\)\*\+\-><~]/', ' ', $attributes['search']);


        $searchTerm = "'+" . str_replace(' ', ' +', $searchValue) . "'";

        // Forzar join con mail_datas si no está definido
        ...
        // FULLTEXT FINAL — (subject, body_plain, body_html)
        // $extra_cond = "
        //     MATCH (jt.subject, jt.body_plain, jt.body_html)
	...
	function check_if_new_mails() {
		...
		try {
			...
			$extra_cond = "received_date > '".$date->toMySQL()."'";
